...
if df_raw is not None:
    ...
    if curr_df.empty:
        ...
        if final_df.empty:
            ...
            with col_right:
                st.subheader("📈 评分历史轨迹")
                trend_player = st.selectbox("选择球员观察", final_df['球员'].unique())
                if trend_player:
                    ...
                    fig_line = px.line(hist_data, x='Fetch_Date', y=strategy_col, markers=True, 
                                       title=f"{trend_player} 评分变动记录",
                                       color_discrete_sequence=[NBA_BLUE])
                    fig_line.update_layout(yaxis_tickformat='.2f')
                    st.plotly_chart(fig_line, use_container_width=True)

            # --- 3.3 评分分布点状扫描图 (修复报错的核心部分) ---
            st.divider()
            st.subheader(f"📍 实时战力评分扫描 ({sel_year}届)")
            
            # 数据预处理：防御性编程，防止 px.strip 因为 size 为 0 或 NaN 报错
            plot_df = final_df.copy()
            plot_df['场均得分'] = pd.to_numeric(plot_df['场均得分'], errors='coerce').fillna(0)
            plot_df['点大小'] = plot_df['场均得分'].apply(lambda x: max(x, 1.0)) # 确保点最小可见且不为0
            
            avg_score = plot_df[strategy_col].mean()
            
            try:
                fig_dot = px.strip(plot_df, x=strategy_col, 
                                  orientation='h',
                                  color='模型信号',
                                  size='点大小', # 使用预处理后的数值
                                  hover_name='球员',
                                  hover_data={strategy_col: ':.2f', '场均得分': ':.1f', '变动趋势': ':.2f', '点大小': False},
                                  color_discrete_map={
                                      "🔥 手感火热": NBA_RED,
                                      "👑 基石表现": NBA_BLUE,
                                      "🆙 状态复苏": "#117a65",
                ...
                fig_dot.add_vline(x=avg_score, line_dash="dash", line_color="gray", 
                                  annotation_text=f"届别平均线: {avg_score:.2f}", 
                                  annotation_position="top left")
                
                fig_dot.update_traces(marker=dict(opacity=0.7, line=dict(width=1, color='White')))
                fig_dot.update_layout(xaxis_title="量化评价总分", yaxis_title="", height=320, showlegend=True)
                st.plotly_chart(fig_dot, use_container_width=True)
            except Exception as e:
                st.error("评分分布图渲染失败，请尝试放宽样本过滤条件。")

    # --- 3.4 届别成色分析 ---
    st.divider()
    st.subheader("📅 届别成色分析 (选秀大年/小年对比)")
...
st.markdown("---")
st.caption("<div style='text-align: center; color: gray;'>© 2026 RookieFlip2x（乐翻新秀）评价系统 | 数据驱动洞察</div>", unsafe_allow_html=True)
